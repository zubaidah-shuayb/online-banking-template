...
-- ==================================================================== --
-- VERIFICATION — the result of this query should be all `true`.
-- ==================================================================== --
select
  exists (select 1 from information_schema.columns
          where table_schema='public' and table_name='profiles' and column_name='preferred_currency') as profile_columns_ok,
  exists (select 1 from storage.buckets where id='avatars' and public) as avatars_bucket_ok,
  (select count(*) from pg_policies where schemaname='storage' and tablename='objects'
     and policyname like 'avatars%') = 4 as avatar_policies_ok,
  exists (select 1 from pg_proc p join pg_namespace n on n.oid=p.pronamespace
          where n.nspname='public' and p.proname='admin_adjust_balance'
            and pg_get_functiondef(p.oid) like '%::public.txn_type%') as admin_rpc_ok,
  (select count(*) from auth.users u
     left join public.profiles pr on pr.id=u.id where pr.id is null) = 0 as every_user_has_profile,
  (select count(*) from auth.users u
     left join public.accounts a on a.user_id=u.id where a.user_id is null) = 0 as every_user_has_account;
